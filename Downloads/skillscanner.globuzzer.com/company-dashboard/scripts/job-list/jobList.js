...
function fetchJobs(companyId){
    ...
    console.log('fetch jobs now')
    var currentTimeInMili = new Date().getTime()

    firestore.collection("job_expired_test").where("companyId", "==", companyId).onSnapshot(function(querySnapshot){
        ...
        querySnapshot.forEach(function(doc){
            if (doc) {
                console.log(new Date(doc.data().deadline).getTime(), new Date(doc.data().expiredAt).getTime())
                console.log(doc.data().deadline, doc.data().expiredAt)
...
function renderJobs(byCategory, byCountry, byDate, jobData){
    ...
    if(jobData.data.category.toLowerCase().trim() === byCategory.toLowerCase().trim() || byCategory.toLowerCase() === "by category" || byCategory.toLowerCase() === "all"){
        if(jobData.data.country.toLowerCase().trim() === byCountry.toLowerCase().trim() || byCountry.toLowerCase() === "by country" || byCountry.toLowerCase() === "all"){
            ...
            // var jobListContentBox = document.createElement('input')

            jobListContentTitle.appendChild(jobListContentTitleP)
            // jobListContentTitle.appendChild(jobListContentBox)
            ...
            jobListContentDelete.onclick = function() { 
                console.log('right after click delete')
            ...
            //             } else if (selectedJobs.some(job => job.jobId === jobItem.jobId)) {
            //                 return;
            //             }
            //         } else {
            //             selectedJobs.push(jobItem);
            ...
            if ($('.job-list-content-holder').children().length > 1) {
                $('.job-list-content-holder .selectAll').removeClass('hide');
                $('.job-list-content-holder .select-option').removeClass('hide');
            } 
        }
    }
}
...
function renderSavedJobs(byCategory, byCountry, byDate, jobData){
    if(jobData.data.category.toLowerCase().trim() === byCategory.toLowerCase().trim() || byCategory.toLowerCase() === "by category" || byCategory.toLowerCase() === "all"){
        if(jobData.data.country.toLowerCase().trim() === byCountry.toLowerCase().trim() || byCountry.toLowerCase() === "by country" || byCountry.toLowerCase() === "all"){
            ...
            checkBox.type = 'checkbox';
            checkBox.name = jobData.id;
            checkBox.classList.add('job-item-saved', 'job-item');
            checkBox.setAttribute('fullPath', jobData.data.fullPath);
            ...
            if ($('.job-list-saved-content-holder').children().length > 1) {
                $('.job-list-saved-content-holder .selectAll').removeClass('hide');
                $('.job-list-saved-content-holder .select-option').removeClass('hide');
            } 
        }
    }
}
    ...
    if (selectedJobs.length > 0) {
        if(confirm("Are you sure to delete these")) {
            selectedJobs.map(job => {
                firestore.collection("job_expired_test").doc(job.jobId).delete().then(function(){
...
function deleteSavedAll () {
    if (selectedJobs.length > 0) {
        if(confirm("Are you sure to delete these")) {
            selectedJobs.map(job => {
                firestore.collection("job_offer_drafts").doc(job.jobId).delete().then(function(){
                    // deleteFromStorage(job.jobFullPath)
                    // .then(function(){
                    // })
                    console.log('dele saved')
                    fetchJobs(window.localStorage.getItem("companyEmail"))
                })
                .catch(function(err){
                    console.error("Error removing document: ", err);
...
function deleteJob(jobId, fullPath){
    if(confirm("Are you sure to delete these")){
        firestore.collection("job_expired_test").doc(jobId).delete().then(function(){
...
function showSavedJobs(){
    if ($('.job-list-saved-content-holder').find('div.job-list-content-row').length !== 0) {
        console.log('hi')
        $('.job-list-edit').css({'display': 'flex'})
    } else {
        $('.job-list-edit').css({'display': 'none'})
    ...
    selectedJobs = [];
    $('.clearAll').attr('class', 'clearAll-saved')
    $('.selectAll').attr('for', 'check-all-saved');
    $('input[type=checkbox]').prop('checked', false);
    $('.job-list-edit input').attr('class', 'selectAllCheckbox-saved');
    $('.job-list-edit input').attr('id', 'check-all-saved');
    $("#job-list-expired-content-holder").removeClass("show-job-list-expired-content-holder")
    $("#job-list-saved-content-holder").removeClass("show-job-list-expired-content-holder")
...
function renderExpiredJobs(byCategory, byCountry, byDate, jobData){
    console.log(jobData.data.fullPath);
    // $("#job-list-content-holder *:not('.edit-tools')").empty()
    // $("#job-list-expired-content-holder *:not('.edit-tools')").empty()
    if(jobData.data.category.toLowerCase().trim() === byCategory.toLowerCase().trim() || byCategory.toLowerCase() === "by category" || byCategory.toLowerCase() === "all"){
        if(jobData.data.country.toLowerCase().trim() === byCountry.toLowerCase().trim() || byCountry.toLowerCase() === "by country" || byCountry.toLowerCase() === "all"){
            ...
            checkBox.type = 'checkbox';
            checkBox.name = jobData.id;
            checkBox.classList.add('job-item-expired', 'job-item');
            checkBox.setAttribute('fullPath', jobData.data.fullPath);
            ...
            if ($('.job-list-expired-content-holder').children().length > 1) {
                $('.job-list-expired-content-holder .selectAll').removeClass('hide');
                $('.job-list-expired-content-holder .select-option').removeClass('hide');
            } 
        }
    }
}
...
function testSaved() {
    testSamples.map(el => {
        firestore.collection('job_offer_drafts').doc(el).set({
            country: 'Finland',
            category: 'IT',
            companyId: 'buiphuquang@gmail.com',
            fullPath: `buiphuquang@gmail.com/${el}`,
            createdAt: new Date()
        },
        { merge: true })
    })
...
function testExpired() {
    testSamples.map(el => {
        firestore.collection('job_expired_test').doc(el).set({
            country: 'Finland',
            category: 'IT',
            companyId: 'buiphuquang@gmail.com',
            fullPath: `buiphuquang@gmail.com/${el}`,
